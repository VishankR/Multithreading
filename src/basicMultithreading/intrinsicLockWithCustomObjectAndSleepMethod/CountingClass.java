...
public class CountingClass extends Counter {
	...
	public boolean incrementCounter1() throws InterruptedException {
		/*
		 * If your resource R is java monitor, then there are only two ways to release
		 * it: 1. exit synchronized block 2. call wait on owned monitor
		 */
...
//           as well whether they are affecting the same variable or not.
		/*
		 * synchronized (this){ while(counter1 <= upperLimit) counter1++; }
		 */
...
//           synchronize on a final Object
		synchronized (obj1) {
			// obj1 = new Object();
			System.out.println(ConsoleTextColors.TEXT_RED + "Have Accquired lock on obj1 and incrementing counter1"
					+ ConsoleTextColors.TEXT_RESET);
			/*
			 * 1. Thread.sleep causes the current thread to suspend execution for a
			 ...
			 * might be running on a computer system. Means During this time, it's not
			 * consuming CPU time, so the CPU can be executing other tasks. 
			 * 2. The thread.sleep() method essentially interacts with the thread scheduler to put
			 * the current thread into a wait state for the required interval. The thread
			 * however does not lose ownership of any monitors.
			 */
			Thread.sleep(2000);
			counter1++;
			return true;
	...
	public boolean incrementCounter2() throws InterruptedException {
		/*
		 * synchronized (CountingClass.class){ counter2++; }
		 */
		synchronized (obj1) {
			System.out.println(ConsoleTextColors.TEXT_BLUE + "Have Accquired lock on obj2 and incrementing counter2"
					+ ConsoleTextColors.TEXT_RESET);
			Thread.sleep(2000);
			counter2++;
			return true;
	...
	public boolean incrementCounter3() throws InterruptedException {
		synchronized (obj1) {
			System.out.println(ConsoleTextColors.TEXT_CYAN + "Have Accquired lock on obj1 and incrementing counter3"
					+ ConsoleTextColors.TEXT_RESET);
			Thread.sleep(2000);
			counter3++;
			return true;
	...
	/*
	 * public static CountingClass getInstance(){ if(null == instance ){ instance =
	 * new CountingClass(); } return instance; }
	 */
